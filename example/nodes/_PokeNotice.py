...
class PokeNotice(Node[PokeNotifyEvent, Any, Any]):
    ...
    async def handle(self) -> None:
        if getattr(self.event, "group_id", None):
            await self.call_api(
                "group_poke", group_id=self.event.group_id, user_id=self.event.user_id
            )
        else:
            await self.call_api("friend_poke", user_id=self.event.user_id)
