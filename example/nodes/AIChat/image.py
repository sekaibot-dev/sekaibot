        ...
        with Image.open(BytesIO(image_bytes)) as img:
            ...
            if img.format == "GIF" and getattr(img, "is_animated", False):
                ...
                if len(image_bytes) > max_gif_size:
                    ...
                    static_frame.save(buffer, format="JPEG", optimize=True, quality=gif_quality)
...
async def fetch_image_as_base64(
    url: str, max_jpg_size: int = 100 * 1024, gif_quality: int = 55, max_gif_size: int = 300 * 1024
) -> str:
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, ssl=ssl_context) as response:
            if response.status == 200:
                ...
                raise Exception(f"Failed to fetch image. HTTP status code: {response.status}")
