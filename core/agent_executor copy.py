    ...
    async def run_chat(
        ...
        async with self.semaphore:  # ✅ 限制并发
            try:
                ...
                response_text: str = response.content if isinstance(response, AIMessage) else str(response)
            except Exception as e:
                response_text = f"Error: {str(e)}"  # ✅ 避免崩溃，返回错误信息

        # ✅ 更新对话历史（追加用户输入 & AI 回复）
        new_history = state.history + [HumanMessage(content=state.input), AIMessage(content=response_text)]
