        ...
        symbol_msg = (
            [
                ...
                *(
                    ...
                    for msg in (
                        HumanMessage(content=convo["input"]),
                        AIMessage(content=convo["output"]),
                    )
                ),
                SystemMessage(content="以上是样例对话"),
            ]
            if self.symbol_conversation is not None and len(self.symbol_conversation) > 0
            else []
        )

        return role_msg + symbol_msg

    def get_system_message(self, user_name: str) -> list[BaseMessage]:
        """
        获取包含系统消息的列表
        """
        now_str = datetime.now().strftime("%Y年%m月%d日 %H点%M分")
        info_msg = [SystemMessage(content=(f"当前时间：{now_str}\n对方名称：{user_name}\n"))]
