...
async def _check_reply(adapter: "CQHTTPAdapter", event: MessageEvent) -> None:
    ...
    try:
        event.reply = TypeAdapter(Reply).validate_python(
            await adapter.get_msg(message_id=int(msg_seg.data["id"]))
        )
    ...
    async def handle_cqhttp_event(self, msg: dict[str, Any]) -> None:
        ...
        if cqhttp_event.post_type == "meta_event":
            ...
            if isinstance(cqhttp_event, MessageEvent):
                await _check_reply(self, cqhttp_event)
                await _check_at_me(self, cqhttp_event)
